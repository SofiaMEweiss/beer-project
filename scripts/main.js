...
let initRandom = () => {
	const sectElement = createNewElement("section", eMain, {
		class: 'indexRandomBeerSection'
	}, "");
	createNewElement("div", sectElement, {
		class: 'showBeerContainer'
	}, "");
	const buttonElement = createNewElement("button", sectElement, {
		type: 'button',
		id: 'button',
		name: 'button'
	}, "Click for a random beer");
...
let displayRandomBeer = (beerName, beerImage, beerId) => {
	...
	const sectionElement = createNewElement("section", divElement, {
		class: 'beercard'
	}, "");
	createNewElement("h2", sectionElement, {}, beerName);
	createNewElement("img", sectionElement, {
		src: beerImage,
		alt: "A random beer"
	}, "");
	const aElement = createNewElement("a", sectionElement, {
		class: "seeMoreLink",
		href: "#"
	}, "See More");
...
let showProduct = (beerId) => {
	if (page_saved == 0) {
		if (randomBeer_saved.length > 0) {
			showProduct2(randomBeer_saved);
		} else {
...
let showProduct2 = (beerId) => {
	//console.log(beerId);
	let eContainer = createNewElement("section", eMain, {
		class: 'indexInfoBeerSection'
	}, "");
	let eContainer2 = createNewElement("div", eContainer, {
		class: 'beerImgContainer'
	}, "");
	createNewElement("h2", eContainer2, {}, beerId[0].name);
	let imgElement = createNewElement("img", eContainer2, {
		class: 'showimg',
		alt: "Picture of bottle",
		src: noPic
	}, "");
	...
	let eContainer3 = createNewElement("article", eContainer, {
		class: 'beerInfoContainer'
	}, "");
	...
	let infoData = [{
			desc: "Description",
			val: beerId[0].description
		},
		{
			desc: "First brewed",
			val: beerId[0].first_brewed
		},
		{
			desc: "Alcohol by volume",
			val: beerId[0].abv
		},
		{
			desc: "Volume",
			val: beerId[0].volume.value + " " + beerId[0].volume.unit
		},
		{
			desc: "Boil volume",
			val: beerId[0].boil_volume.value + " " + beerId[0].boil_volume.unit
		},
		{
			desc: "Ingredients",
			val: "MALTS: " + malt_add + " YEAST: " + beerId[0].ingredients.yeast
		},
		{
			desc: "Hops",
			val: hops_add
		...
		{
			desc: "Brewers tips",
			val: beerId[0].brewers_tips
		}
	];

	for (let i = 0; i < infoData.length; i++) {
		let optElement = createNewElement("li", infoElement, {
			class: 'showBeerOptions'
		}, infoData[i].desc + ": ");
		let hElement = createNewElement("span", optElement, {
			class: 'beerInfo'
		}, infoData[i].val);
	}
}
...
let createNewElement = (type, parentElement, attributes, txtcont) => {
	...
	if (txtcont.length > 0) {
		createdElement.textContent = txtcont;
	}
...
let nysida = (param) => {
	...
	switch (param) {
		case 0:
			initRandom();
			if (randomBeer_saved.length > 0) {
				createRandomBeer(randomBeer_saved);
			}
